...
<div class="auth-card" style="max-width:760px;grid-template-columns:300px 1fr;box-shadow:0 24px 64px rgba(16,185,129,.15),0 4px 20px rgba(6,182,212,.10);">
  ...
  <div class="form-panel" style="justify-content:center;">
    ...
    <div class="icon-ring" style="background:linear-gradient(135deg,#10b981,#06b6d4);box-shadow:0 8px 24px rgba(16,185,129,.38);">
    ...
    <form method="POST" action="{{ route('password.store') }}" id="resetForm" novalidate>
      ...
      <div class="field-wrap">
        ...
        <input type="email" id="email" name="email"
          ...
          placeholder="nama@instansi.go.id" required readonly
      ...
      <div class="field-wrap" id="wrap-cpw">
        ...
        <div class="field-error" id="err-cpw">Password tidak cocok.</div>
      </div>
...
document.getElementById('password').addEventListener('input',function(){
  const v=this.value; let s=v.length;
  ...
  const toggle=(id,ok)=>{ const el=document.getElementById(id); el.classList.toggle('ok',ok); };
...
document.getElementById('resetForm').addEventListener('submit',function(e){
  ...
  if(pw!==cpw){ e.preventDefault(); err.style.display='block'; wrap.classList.add('has-error'); return; }
  err.style.display='none'; wrap.classList.remove('has-error');
